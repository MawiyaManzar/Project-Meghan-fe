    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-slate-50 text-slate-800">
        <div className="max-w-md w-full text-center space-y-12">
          ...
          <button onClick={handleOrbContinue} className="group flex items-center gap-2 mx-auto px-10 py-5 bg-white text-slate-700 rounded-full shadow-lg hover:shadow-xl hover:scale-105 transition-all font-bold">
        ...
        <div className="max-w-lg w-full space-y-8 glass p-10 rounded-[48px] shadow-2xl">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[StressSource.ACADEMICS, StressSource.RELATIONSHIP, StressSource.FAMILY, StressSource.OTHERS].map((s) => (
              <button key={s} onClick={() => s !== StressSource.OTHERS ? handleTriageContinue(s) : setUserState({...userState, source: StressSource.OTHERS})} className={`p-6 rounded-[32px] border text-left transition-all duration-300 ${userState.source === s ? 'bg-teal-500 text-white border-teal-600 shadow-xl shadow-teal-100 scale-[1.02]' : 'bg-white text-slate-600 border-slate-100 hover:border-teal-200'}`}>
          ...
          {userState.source === StressSource.OTHERS && (
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <textarea className="w-full p-5 rounded-[32px] border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-400 min-h-[120px] text-sm leading-relaxed" placeholder="Briefly share what's happening..." onChange={(e) => setUserState({...userState, otherText: e.target.value})} />
              <button onClick={() => handleTriageContinue(StressSource.OTHERS, userState.otherText)} className="w-full py-5 bg-teal-500 text-white rounded-full font-bold shadow-xl shadow-teal-200 hover:bg-teal-600 transition-all active:scale-95">Find Clarity</button>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="flex items-center gap-3">
             ...
             >
               ...
               <div className="w-10 h-10 bg-teal-500 rounded-full flex items-center justify-center text-white font-bold shadow-lg shadow-teal-100 group-hover:scale-105 transition-transform">
                 {userState.bio.name ? userState.bio.name.charAt(0).toUpperCase() : <User size={20} />}
               </div>
             </button>
          </div>
        </header>
...
const MobileNavBtn: React.FC<{active: boolean, onClick: () => void, icon: any}> = ({ active, onClick, icon }) => (
  <button onClick={onClick} className={`p-3.5 rounded-full transition-all duration-300 ${active ? 'bg-teal-500 text-white shadow-xl shadow-teal-200' : 'text-slate-400 hover:bg-slate-50'}`}>
    {icon}
  </button>
);
